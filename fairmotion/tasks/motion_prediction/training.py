...
from fairmotion.tasks.motion_prediction.joint_loss import PerJointLoss
from fairmotion.tasks.motion_prediction.dataset import sequential_dataset
...
def train(args):
    ...
    if args.architecture == 'st_transformer':
        criterion = PerJointLoss(device=device)
    else:
    ...
    for iterations, (src_seqs, tgt_seqs) in enumerate(dataset["train"]):
        model.eval()
        for src_seqs_i, tgt_seqs_i in sequential_dataset(src_seqs, tgt_seqs):
            src_seqs_i, tgt_seqs_i = src_seqs_i.to(device), tgt_seqs_i.to(device)
            outputs = model(src_seqs_i, tgt_seqs_i, teacher_forcing_ratio=1,)
            loss = criterion(outputs, tgt_seqs_i)
    ...
    for epoch in range(args.epochs):
        ...
        for iterations, (src_seqs, tgt_seqs) in enumerate(dataset["train"]):
            for src_seqs_i, tgt_seqs_i in sequential_dataset(src_seqs, tgt_seqs):
                ...
                src_seqs_i, tgt_seqs_i = src_seqs_i.to(device), tgt_seqs_i.to(device)
                outputs = model(
                    src_seqs_i, tgt_seqs_i, teacher_forcing_ratio=teacher_forcing_ratio
                )
                outputs = outputs.double()
                loss = criterion(
                    outputs,
                    utils.prepare_tgt_seqs(args.architecture, src_seqs_i, tgt_seqs_i),
                )
                loss.backward()
                opt.step()
                epoch_loss += loss.item()
        # todo figure out how to scale with accordingly to the additional sequences
        epoch_loss = epoch_loss / num_training_sequences
