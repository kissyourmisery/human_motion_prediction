...
def calculate_metrics(pred_seqs, tgt_seqs):
    ...
    R_pred, p_pred = conversions.T2Rp(pred_seqs)
    R_tgt, p_tgt = conversions.T2Rp(tgt_seqs)

    errors = dict()

    pred_euler, tgt_euler = metrics.pred_targ_2_euler(
        R_pred[:, :, amass_dip.SMPL_MAJOR_JOINTS],
        R_tgt[:, :, amass_dip.SMPL_MAJOR_JOINTS],
    )
    # Euler Angle Difference using Dynamic Time Warping (DTW)
    errors["dtw_euler_diff"] =  dtw(pred_euler, tgt_euler)
    ...
    # Mean-Angle Error (MAE)
    errors["mae"] = {frame: np.sum(euler_error[:frame]) for frame in metric_frames}
...
def test_model(model, dataset, rep, device, mean, std, max_len=None):
    ...
    if len(pred_seqs) > 0 and pred_seqs[0].shape == tgt_seqs[0].shape:
        errors = calculate_metrics(seqs_T[0], seqs_T[2])
    return seqs_T, errors
