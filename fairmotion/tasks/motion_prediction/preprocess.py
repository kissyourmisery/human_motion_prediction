...
if __name__ == "__main__":
    ...
    # Edited by Sean Crutchlow (scrutchlow3): create a to specify data set size
    parser.add_argument(
        "--dataset-size",
        type=str,
        help="AMASS-DIP data set size to be used.",
        choices=["small", "medium", "large", "complete"],
        default="complete",
    )
    parser.add_argument(
        "--rep",
    ...
    # Edited by Sean Crutchlow (scrutchlow3): load specified data set size
    prefix = "" if args.dataset_size == "complete" else args.dataset_size + "_"

    train_files = read_content(
        os.path.join(args.split_dir, prefix + "training_fnames.txt")
    )
    validation_files = read_content(
        os.path.join(args.split_dir, prefix + "validation_fnames.txt")
    )
    test_files = read_content(os.path.join(args.split_dir, prefix + "test_fnames.txt"))
